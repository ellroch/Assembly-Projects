...
_start:
	...
	; if ArgC == 3 take third arg as fileName to read from...
	; make a FilePtr from it... (it being ArgV[2]) else default to stdin -- might be a bit of a cheat ~~ but it saves time from having to tell the user they are doing something wrong... - yea definitely a lazy cheat but it works right?

	mov edx, [ebp+4] ; not sure if cmp can use a memory source address, so just going to do this to save time
	cmp edx, 3
	jne _default_stdin
...
; if file name specified:
	add eax, 4
	push fopen_mode
	push eax ; push (char*) Argv[2] = fileName
	call fopen ; will return FILE* in eax... woot, can use this in place of stdin
	pop ecx ; using ecx to cean up stack
	pop ecx ;
	push eax ; eax currently has FILE*
	...
	;	ebp -12= (FILE*) file named by ArgV[2]
	jmp _continue
